#!/bin/bash
# Output star gains for daily report
# Usage: GITHUB_TOKEN=xxx ./star-report.sh
...
# If old data exists, calculate gains and output report
if [ -f "$DATA_FILE" ]; then
  python3 << 'PYEOF'
import json
import os

DATA_FILE = os.path.expanduser("~/.clawdbot/star-tracker.json")
...
# Cleanup: keep only latest as previous
import shutil
shutil.copy(DATA_FILE + ".new", DATA_FILE)
PYEOF
fi

# Update data file if no old data
if [ ! -f "$DATA_FILE" ]; then
